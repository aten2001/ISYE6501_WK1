...
Judging by the naming scheme from the header, `R1` is the response and `Ai` are the predictors. The response is reformated to be either -1 or 1 and converted to a factor.
...
knn_data[, 11] = factor(knn_data[, 11])

knn_model <- train.kknn(R1 ~ ., data = knn_data)
